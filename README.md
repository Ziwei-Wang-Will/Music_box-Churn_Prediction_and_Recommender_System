...
X music box is a well-known music streaming service and interested in **Churn Prediction and Recommendation**. To help explore this question, they have provided log data containing billions of song play, search, and download records generated by 600K users. 

## Dataset description
- Play log data
  - 115 million 'Play' log records.
  - 'uid', 'device', 'song_id', 'song_type', 'song_name', 'singer', 'play_time', 'song_length', 'paid_flag', 'date'
- Download log data
  - 8 million 'Download' log records.
  - 'uid', 'device', 'song_id', 'song_name', 'singer', 'paid_flag', 'date'
- Search log data
  - 9 million 'Search' log records.
...
- **Applied downsampling on uid level** 
...
- **Label definition**
  ...
  - we had 36,272 churn label and 22,160 active label, two classes were comparable.
- **Data cleaning**
  - Data processing of "song_length" 
  - Data cleaning for "play_time"
- **Feature generation**
  - Generated Features A: Play time percentage related features
    - Generated features **play time percentage proportion feature**:
      ...
      - **Note**: for each ‘uid’, the sum of five features above was 1.
      - **For example**: ‘time_percentage_0_to_20’ meant 0<=percentage<20, which was ratio of (number of played songs with play time percentage less than 20%) to (total number of played songs) per ‘uid’
    - Generated features **Acceleration features of different level of play time percentage**: Ratio of count of songs played with >=80 percentage of nearest 7 days to that of nearest 30 days.
  - Generated Features B: Play time percentage related features
    - Generated features **Total play_time**: Total play time per ‘uid’.
    - Generated features **Acceleration features of total play_time**: Ratio of total play time of nearest 7 days to that of nearest 30 days. 
      - If the total play time acceleration ratio was less than 25%, meant user had played less time in the last 7 days than average level of the last 30 days.
    - Generated features **Average play_time of played songs**: Average play times of songs per ‘uid’.
  - Generated Features C: Event related features
    - Generated features **events frequency in given windows**: Count of ‘P’, ‘D’ and ‘S’ in given windows per ‘uid’.
    - Generated features **Acceleration features of events**: Ratio of event frequency of nearest 7 days to that of nearest 30 days. 
      - If the total play time acceleration ratio was less than 25%, meant user had operated less frequently in the last 7 days than average level of the last 30 days.
  - Generated Features D: Recency related features
    - Generated features **Last Event Time from feature_window_end_date**: Measured the time gap between ‘the last active day’ and ‘feature_window_end_date’.
  - Generated Features E: Profile related features
    - Generated features **device_feature**: Device type per ‘uid’.
      - We found 20 users use multiple devices, we assigned the device label as device with more entries by correspond user, if user had the same entries number of 'android' and 'iphone', we assigned its device label as 'iphone'.
- **Formed training data for prediction models**
- **Formed rating data for recommender systems**
  - We defineed **rating** as **max{‘play_score’, ‘download_score’}**: 
    - **play_score** were generate by ‘play_time_percentage_of_song_length’ with idea that the larger played percentage was, the more likely the user liked the song, rules as below: 
      - 0.8 <= ‘play_time_percentage_of_song_length’, assigned ‘play_score’ 5
      - 0.6 <= ‘play_time_percentage_of_song_length’ < 0.8, assigned ‘play_score’ 4
      - 0.4 <= ‘play_time_percentage_of_song_length’ < 0.6, assigned ‘play_score’ 3
      - 0.2 <= ‘play_time_percentage_of_song_length’ < 0.4, assigned ‘play_score’ 2
      - 0 <= ‘play_time_percentage_of_song_length’ < 0.2, assigned ‘play_score’ 1
      - **Note**: If per uid per song_id had multiple ratings, we toke average.
...
- **Models comparison and reasoning**
  - **Logistic Regression**
    ...
    - **AUC** of test data was **0.8866** with **Logistic Regression**.
  ...
  - **Random Forest**
    ...
    - **AUC** of test data was **0.9061** with **Random Forest**, better than that of **Logistic Regression** with **0.8866**.
      - **Reason**: There were feature interaction and non-linearity relationship between features and target in our data set, trees algorithms can deal with these problems while logistic regression couldn't.
  ...
  - **Gradient Boosting Trees**
    ...
    - **AUC** of test data was **0.9036** with **Gradient Boosting Trees**, was close to that of **Random Forest** with **0.9061**.
    ...
    - Then we tried HyperParameter Tuning with Grid Search for Random Forest, to figure out whether we could do better.
...
- **Top 10 features analysis**
  ...
  - **last_P_time_from_2017-04-28**: the larger the time gap between 'the last active day' and 'feature_window_end_date' was, the more likely the user would churn.   
  - **freq_P_last_7**: the smaller play frequency in the last 7 days was, the more likely the user would churn. And its feature importance was larger than those of 'freq_P_last_14', 'freq_P_last_30', which showed user recent behavior pattern was more informative.  
  - **freq_P_last_14**: the same idea with 'freq_P_last_7'  
  - **total_play_time_7d_over_30d**: if the total play time acceleration ratio was less than 25%, meant user had played less time in the last 7 days than average level of the last 30 days, the smaller the ratio was, the more likely the user would churn. 
  - **time_percentage_larger_than_80_7d_over_30d**: if the time_percentage acceleration ratio was less than 25%, means user had played less songs with high percentage in the last 7 days than average level of the last 30 days, the smaller the ratio was, the more likely the user would churn.   
  - **P_7d_over_30d**: if the play frequency acceleration ratio was less than 25%, means user had played less frequently in the last 7 days than average level of the last 30 days, the smaller the ratio was, the more likely the user would churn.   
  - **total_play_time**: the smaller total play time was, the more likely the user would churn.   
  - **freq_P_last_30**: the same idea with 'freq_P_last_7'.  
  - **freq_S_last_14**: the smaller search frequency in the last 14 days was, the more likely the user would churn. Its feature importance was large perhaps because 14 days fits our label window which last 14 days better.  
  - **freq_P_last_3**: the same idea with 'freq_P_last_7', while its time window was too short to be as informative as 'freq_P_last_7'  
...
- **Cleaned data and created utility matrix**
  - We built utility matrix with only users played more than five songs.
  - For the removed or new users, we could recommend popular songs at first.
- **Build Recommender Systems**
  - **Popularity-based recommender**
    - We defined **popular** as songs with most played records.
    - For every new user or user played less than five songs, we built a **Popularity-based recommender** to recommend most popular songs at first. 
  ...
  - **Neighborhood-based Approach Collaborative Filtering Recommender：Item-Item similarity recommender**
    ...
    - Given a user_id and recommend 10 songs which had the largest similarities with songs the user had played before.
  ...
  - **Matrix Factorization Approach Collaborative Filtering Recommender：NMF**
    - The **RMSE** of **NMF** was **1.3494**, and the **average absolute error** was **0.6121**, the performance was acceptable. 
    ...
    - The same as what we discussed above, the **average absolute error** of **NMF** for this specific user was better than **0.9656** of **Item-Item similarity recommender**.
  ...
  - **Matrix Factorization Approach Collaborative Filtering Recommender：UVD**
    - The **RMSE** of **UVD** was **1.1729** and the **average absolute error** was **0.5569**, which were better than scores of **NMF**(**1.3494** and **0.6121**). 
...
- **Recommendation results Analysis between different recommendation systems**
  ...
  - From the overlap tables above, we notice that:
    - The recommended songs given by **Popularity-based**, **Neighborhood-based approach** and **Matrix Factorization approach** models were very different from each other, have no overlap in top 10 and only 3 overlaps in top 100 recommended songs.
    - While the recommended songs given by **NMF** and **UVD** had 4 overlaps in top 10 and 66 overlaps in top 100 recommended songs.
- **Conclusion**
